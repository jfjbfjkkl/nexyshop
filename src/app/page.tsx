...
import { sortByDailyPopularity, trackProductEvent, type PopularityProductType } from "@/lib/popularity";
...
const mobilePopularGames = [...popularGames].reverse();
...
const giftCardsHome: GiftCardHome[] = [
    ...
    const updatePopularSelections = () => {
      ...
      setVisibleGames(
        isMobile
          ...
          : sortByDailyPopularity(popularGames, "game", (game) => game.slug).slice(0, 5),
      );
      setVisibleGiftCards(sortByDailyPopularity(giftCardsHome, "giftcard", (card) => card.slug).slice(0, 4));
    };
              ...
              <div className="desktop-search-results">
                {searchGames.map((game) => (
                  <Link className="desktop-search-result" href={`/jeux/${game.slug}`} key={game.slug} onClick={() => setSearchOpen(false)}>
                    <span className="search-thumb" style={{ "--art": game.art } as CSSProperties} />
                    <span>
              ...
              <div className="desktop-game-grid">
                {(viewMode === "games" ? (isMobileHome ? mobilePopularGames : popularGames) : visibleGames).map((game) => (
                  ...
                  >
                    ...
                    >
                      <div className="desktop-game-art"><GameMark name={game.name} /></div>
